...
Elliot is a open source system for managing containerized applications on top of the IoT device with an emphasis to usability, simplicity and security. Elliot gives simplified app delivery, isolation and additional security to connected device solutions.

In consideration of connected device limitations like unstable connection, limited computing resources, hardware connectivity, Elliot connects devices to single easy to use platform where you can manage devices and applications easily and safely.

Cloud Native technologies like Docker and Kubernetes have inspired heavily and if you're familiar with those, you find really easy to get started with Elliot.
...
And that's the day when Elliot were born ❤︎
...
Elliot don't try to provide this kind of features at all, actually you can use any cloud service with Elliot.
Elliot provides a easy way to deliver your cloud integration to the device and gives you a way to update the software across thousands of devices safely and easily.
...
## Getting Started
In this guide you will learn basic concepts and learn how you can deploy containers to the device with Elliot.

### Core Concepts
To work with Elliot, you describe the _desired state_ and Elliot Agent works to make the device _current state_ match with _desired state_.

With Elliot, you run all your processes in containers. Containers gives easy software delivery, network, disk and resources isolation.
...
Pod is a basic building block of Elliot and is the main model object what you deploy and manage.
If you're familiar with Kubernetes definition of _Pod_, in Elliot it's exactly same except some features are not yet implemented.
...
Easiest way to test run Elliot is to run ElliotOS in RaspberryPI 3b.

ElliotOS is minimal Linux Operating System which contains only minimal components to run Elliot which are Linux kernel, runc, containerd and `elliotd` daemon. Check the [ElliotOS]() section for more info.

1. Install [Etcher CLI](https://etcher.io/cli/)
2. Download ElliotOS image
3. Flash the ElliotOS image to SD card
```shell
etcher elliotos.img
```
...
### Deploy first app
When you have connected RaspberryPI to network and power it on, first step is to check that Elliot can discover and connect to the device.
```shell
eli get devices
```
...
This was the quick start how to deploy containers to the Elliot. Next step is to learn [how you can develop your software in the device in real time]()
...
For this Elliot offer really easy way, `run` -command.
...
If you type `exit` your terminal session comes back to your local computer and Elliot removes all created containers.
...
### `eli get devices`
Elliot can search devices automatically from network with mDNS protocol. You can get list of devices what Elliot finds with `get devices` command.
...
```
After this, Elliot will stop and remove all container(s) from the device and free the used resources.
...
### `eli run`
When you develop your software, often you need to have access to the device to connect your software to some hardware sensor. To make development as easy as possible, Elliot have `run` command.
...
sync:
  target: /go/src/github.com/ernoaapa/elliot
binds:
  - /dev:/dev
```

## ElliotOS
ElliotOS is minimal Linux Operating System, built with [Linuxkit](https://github.com/linuxkit/linuxkit), which contains only minimal components to run Elliot.

- `Linux kernel`
- `containerd` - Container runtime
- `runc` - Run containers based on OCI specification
- `elliotd` - Primary node agent which manages containers based on specs

There's more info in [project GitHub page](https://github.com/ernoaapa/elliot-os) and pre-built images available in [releases page](https://github.com/ernoaapa/elliot-os/releases).